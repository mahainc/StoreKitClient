...
- **`StoreKitClient`** — interface: `loadProducts`, `purchase`, `restorePurchases`, `observeTransactions`, `getLatestTransaction`, `isEligibleForIntroOffer`, `processUnfinishedConsumables`, `canMakePayments`, `requestReview`, `receiptURL`, plus an `extension StoreKitClient.Product` value type with `priceFormatStyle` for localized currency display (added in 1.1.0).
- **`StoreKitClientLive`** — `StoreKit` wrapper with an actor-backed transaction listener registry; registers the live `DependencyKey`.

## Installation

```swift
.package(url: "https://github.com/mahainc/StoreKitClient.git", from: "1.1.0"),
```
